...
import {
    getStatusLabel, getSeverityLabel, formatHealthScore, formatFailureProbability, formatDateTime, calculatePowerOutput
} from '../utils/helpers';

const TABS = {
    OVERVIEW: 'overview', POWER: 'power', HEALTH: 'health', ALERTS: 'alerts', MAINTENANCE: 'maintenance'
};
...
function TurbineDetails({turbine: initialTurbine, onClose}) {
    ...
    useEffect(() => {
        async function loadData() {
            try {
                ...
                const [turbineData, alertsData, predictionsData, eventsData, logsData] = await Promise.all([fetchTurbine(initialTurbine.id), fetchTurbineAlerts(initialTurbine.id).catch(() => []), fetchTurbineHealthPredictions(initialTurbine.id, 10).catch(() => []), fetchTurbineMaintenanceEvents(initialTurbine.id, 10).catch(() => []), fetchTurbineLogs(initialTurbine.id, 100).catch(() => [])]);
    ...
    if (loading) {
        return (<div className={`turbine-details-panel active`}>
    ...
    return (<div className={`turbine-details-panel active`}>
        ...
        <div className="turbine-details-content">
            {activeTab === TABS.OVERVIEW && (<div>
                <div className="overview-grid">
                    ...
                    <div className="overview-card">
                        ...
                        <div
                            className={`card-value ${latestPrediction ? (latestPrediction.health_score > 0.7 ? 'success' : latestPrediction.health_score > 0.4 ? 'warning' : 'danger') : ''}`}>
                    ...
                    <div className="overview-card">
                        ...
                        <div
                            className={`card-value ${latestPrediction ? (latestPrediction.failure_probability < 0.2 ? 'success' : latestPrediction.failure_probability < 0.5 ? 'warning' : 'danger') : ''}`}>
                    ...
                    <div className="overview-card">
                        ...
                        <div
                            className={`card-value ${activeAlerts.length === 0 ? 'success' : activeAlerts.filter(a => a.severity === 'critical').length > 0 ? 'danger' : 'warning'}`}>
                    ...
                    <div className="chart-container">
                        ...
                        <ResponsiveContainer width="100%" height={300}>
                            <AreaChart data={powerChartData}>
                                ...
                                <Tooltip/>
                                <Legend/>
                                <Area type="monotone" dataKey="power" stroke="#0066cc" fill="#0066cc"
                                      fillOpacity={0.6} name="Power Output (kW)"/>
                            </AreaChart>
                        </ResponsiveContainer>
                    </div>
                    <div className="chart-container">
                        ...
                        <ResponsiveContainer width="100%" height={300}>
                            <LineChart data={powerChartData}>
                                <CartesianGrid strokeDasharray="3 3"/>
                                <XAxis dataKey="power"
                                       label={{value: 'Power (kW)', position: 'insideBottom'}}/>
                                <YAxis label={{
                                    value: 'Rotor Speed (rpm)', angle: -90, position: 'insideLeft'
                                }}/>
                                <Tooltip/>
                                <Legend/>
                                <Line type="monotone" dataKey="rotorSpeed" stroke="#00a8e8"
            ...
            {activeTab === TABS.HEALTH && (<div>
                {predictions.length > 0 ? (<>
                    <div className="health-summary">
                        <div className="health-metric-card">
                            <div className="metric-label">Current Health Score</div>
                            <div
                                className={`metric-value large ${latestPrediction.health_score > 0.7 ? 'success' : latestPrediction.health_score > 0.4 ? 'warning' : 'danger'}`}>
                        ...
                        <div className="health-metric-card">
                            ...
                            <div
                                className={`metric-value large ${latestPrediction.failure_probability < 0.2 ? 'success' : latestPrediction.failure_probability < 0.5 ? 'warning' : 'danger'}`}>
                                {formatFailureProbability(latestPrediction.failure_probability)}
                            </div>
                        </div>
                    </div>
                    {latestPrediction.predicted_failure_window_start && (<div className="failure-window-alert">
                    ...
                    {healthChartData.length > 0 && (<div className="chart-container">
                        <h4>Health Trend</h4>
                        <ResponsiveContainer width="100%" height={300}>
            ...
            {activeTab === TABS.ALERTS && (<div>
                {alerts.length === 0 ? (<div className="empty-state">
                    ...
                    {activeAlerts.length > 0 && (<div className="alerts-section">
                            ...
                            {activeAlerts.map(alert => (
                                <div key={alert.id} className={`alert-item-compact ${alert.severity}`}>
                                    <div className="alert-header-compact">
                                        ...
                                        <span
                                            className="alert-time-compact">{formatDateTime(alert.created_at)}</span>
                                    </div>
                    ...
                    {alerts.filter(a => a.acknowledged).length > 0 && (<div className="alerts-section">
                        <h4>Acknowledged ({alerts.filter(a => a.acknowledged).length})</h4>
                        <div className="alerts-list-compact">
                            {alerts.filter(a => a.acknowledged).map(alert => (<div key={alert.id}
                                                                                   className={`alert-item-compact acknowledged ${alert.severity}`}>
                                <div className="alert-header-compact">
                                                        <span className={`alert-severity-badge ${alert.severity}`}>
                                                            {getSeverityLabel(alert.severity)}
                                                        </span>
            ...
            {activeTab === TABS.MAINTENANCE && (<div>
                {events.length === 0 ? (<div className="empty-state">
                    <div className="empty-title">No Maintenance Records</div>
                    <div className="empty-message">No maintenance events have been recorded for this
                        turbine.
                    </div>
                </div>) : (<div className="maintenance-list-compact">
                    {events.map(event => (<div key={event.id} className="maintenance-item-compact">
                        <div className="maintenance-header-compact">
                            <span className="event-type-badge">{event.event_type}</span>
                            <span
                                className="event-time-compact">{formatDateTime(event.start_time)}</span>
                        </div>
                        {event.description && (<div className="event-description-compact">{event.description}</div>)}
                        <div className="event-details-compact">
                            ...
                            {event.cost && (<span>Cost: €{parseFloat(event.cost).toFixed(2)}</span>)}
                            {event.parts_replaced && (<span>Parts: {event.parts_replaced}</span>)}
                        </div>
                    </div>))}
                </div>)}
            </div>)}
